...
class TestAgeWizard(object):
    def test_init_basic(self):
        assert au.AgeWizard(obs_df=fake_hrd_input, model=hr_file), "Loading from file path failed"
...
class TestFindCoordinates(object):
    ...
    def test_cmd_input(self):
        ...
        assert np.sum(np.isclose([col_coord[0], col_coord[1], col_coord[2]], [27, 35, 27]))==3, "color coordinates wrong"
        assert np.sum(np.isclose([mag_range[0], mag_range[1], mag_range[2]], [90, 40, 130]))==3, "magnitude coordinates wrong"
