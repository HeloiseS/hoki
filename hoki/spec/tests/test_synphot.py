...
class TestImportCustomFilter():
    ...
    def test_input_file_name(self):
        ...
        with pytest.raises(FileNotFoundError):
            synphot.import_custom_filter(f" ")

    bp = synphot.import_custom_filter(f"{data_path}/LICK.LICK.U.xml")
...
class TestLoadBandpass():
    ...
    def test_multi_builtin_filter(self):
        bp = synphot.load_bandpass("acs,hrc,f220w")
    ...
    bp1 = synphot.load_bandpass(f"{data_path}/LICK.LICK.U.xml")
    bp2 = synphot.import_custom_filter(f"{data_path}/LICK.LICK.U.xml")
...
class TestBPASStoPSPSpectrum():
    ...
    def test_input_type(self):
        with pytest.raises(TypeError):
            synphot.bpass_to_psp_spectrum(1,[10,10])

    def test_input_pandas(self):
        sp = synphot.bpass_to_psp_spectrum(data.WL,data["6.0"])
    ...
    def test_input_numpy(self):
        sp = synphot.bpass_to_psp_spectrum(data.WL.values, data["6.0"])
        assert type(sp) == psp.spectrum.ArraySourceSpectrum,\
...
class TestApplyBandpass():
    ...
    def test_input_type(self):
        with pytest.raises(TypeError):
            synphot.apply_bandpass(np.arange(0,10,1), 1)
        with pytest.raises(TypeError):
            synphot.apply_bandpass(self.bp, self.sp)

    obs = synphot.apply_bandpass(sp, bp)
...
class TestFlux2VegaMag():
    ...
    def test_input_type(self):
        with pytest.raises(TypeError):
            synphot.flux_to_vegamag([10,1], np.arange(0,10,1))

    def test_zeropoint(self):
        d2vega = 7.767 # pc (from doi:10.1088/0004-6256/136/1/452)
        vega = psp.Vega*4*np.pi*(d2vega*3.0857e18)**2
        bp = synphot.load_bandpass("johnson,v")
        obs = synphot.apply_bandpass(vega, bp)
        assert np.isclose(synphot.flux_to_vegamag(bp, obs, d2vega),0),\
